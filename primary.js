const WebSocket = require('ws');const InputEvent = require('input-event');
...
function findKeyboardDevices() {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        lines[i].includes('Logitech K400') ||
        lines[i].includes('K400 Plus') ||
        lines[i].includes('INSTANT USB Keyboard')
      ) {
        foundKeyboard = true;
  ...
  try {
    ...
    keyboardDevices.forEach((device, index) => {
      try {
        ...
        console.log(
          `Successfully initialized keyboard ${index + 1}: ${device.name} at ${
            device.path
          }`,
        );
        ...
        keyboard.on('keypress', async (event) => {
          try {
            ...
            // Handle 'd' key (code 32) for boss death
              ...
              console.log(
                `Boss death sequence triggered by 'd' key on ${device.name}`,
              );
            ...
            // Handle 'r' key (code 19) for reset
              ...
              console.log(
                `System reset triggered by 'r' key on ${device.name}`,
              );
              await handleReset();
            }
            // Handle 'o' key (code 24) for power off
              ...
              console.log(
                `Shield power down triggered by 'o' key on ${device.name}`,
              );
...
function handleEnterKey(event, keyboardIndex) {
  ...
  if (event.value === 1) {
    ...
    console.log(
      `Primary button PRESSED DOWN on ${keyboardName} at`,
    ...
    setTimeout(() => {
      if (
        primaryButtonStates[keyboardIndex].pressed &&
        ...
        console.log(
          `Reset primary button state due to timeout on ${keyboardName}`,
        );
    ...
    if (
      secondaryButtonState.pressed &&
      state.shieldState === ShieldState.ACTIVE
    ) {
      const timeDiff = Math.abs(
        primaryButtonStates[keyboardIndex].pressTime -
          secondaryButtonState.pressTime,
      );
      console.log(
        `Both buttons are pressed! ${keyboardName} with Secondary, Time difference:`,
        timeDiff,
        'ms',
      );
      if (timeDiff < 500) {
        console.log(
          `Time difference within threshold, shutting down the shield! (Triggered by ${keyboardName})`,
        );
        triggerLights();
      } else {
    ...
    primaryButtonStates[keyboardIndex].pressed = false;
    console.log(
      `Primary button RELEASED on ${keyboardName} after`,
      Date.now() - primaryButtonStates[keyboardIndex].pressTime,
      'ms',
    );
    primaryButtonStates[keyboardIndex].pressTime = null;
    ...
    if (
      ...
      console.log(
        `Reset secondary button state due to timeout after primary release on ${keyboardName}`,
      );
  ...
  try {
    ...
    wss.on('connection', (ws, req) => {
      ...
      ws.on('message', (message) => {
        ...
        try {
          ...
          if (data.buttonPressed) {
            ...
            console.log(
              'Secondary button PRESSED DOWN at',
              new Date().toISOString(),
            );
            ...
            const anyPrimaryPressed = primaryButtonStates.some(
              (state) => state.pressed,
            );
            const firstPressedTime =
              primaryButtonStates.find((state) => state.pressed)?.pressTime ||
              null;

            if (anyPrimaryPressed && state.shieldState === ShieldState.ACTIVE) {
              const timeDiff = Math.abs(
                firstPressedTime - secondaryButtonState.pressTime,
              );
              console.log(
                'Both buttons are pressed! Time difference:',
                timeDiff,
                'ms',
              );
              if (timeDiff < 500) {
                console.log(
                  'Time difference within threshold, triggering lights!',
                );
                triggerLights();
              } else {
            ...
            secondaryButtonState.pressTime = null;
            console.log(
              'Secondary button RELEASED after',
              Date.now() - secondaryButtonState.pressTime,
              'ms',
            );
            ...
            const anyStuckButtons = primaryButtonStates.some(
              (state) => state.pressed && Date.now() - state.pressTime > 1000,
            );
            if (anyStuckButtons) {
      ...
      ws.on('close', (code, reason) => {
        console.log(
          `Secondary Pi disconnected - Code: ${code}, Reason: ${reason}`,
        );
        secondaryButtonState.pressed = false;
        secondaryButtonState.pressTime = null;
...
async function controlWLED(state, options = {}) {
  console.log(
    `Attempting to set WLED state to: ${state ? 'ON' : 'OFF'} with options:`,
    options,
  );
  try {
...
async function triggerLights() {
  ...
  try {
    ...
    for (
      let i = 0;
      i < SHATTER_FLICKERS && state.shieldState === ShieldState.SHATTERING;
      i++
    ) {
      // Random duration for this flicker
      const flickerDuration = Math.floor(
        Math.random() * (FLICKER_DURATION_MAX - FLICKER_DURATION_MIN) +
    ...
    for (
      let i = FADE_STEPS;
      i >= 0 && state.shieldState === ShieldState.SHATTERING;
      i--
    ) {
      ...
      if (state.shieldState === ShieldState.BROKEN) {
        ...
        for (
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, intervals[i] * scaleFactor),
          );
        }
        ...
        if (state.shieldState === ShieldState.REGENERATING) {
          ...
          resetAllButtonStates();
          console.log(
            'Shield regeneration complete, boss is invulnerable again',
          );
        }
      }
      regenerationTimer = null;
