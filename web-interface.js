...
let gameState = {
  primaryButton: { pressed: false, pressTime: null, details: [] },
  secondaryButton: { pressed: false, pressTime: null },
  shieldStatus: 'on',
  isRegenerating: false,
  currentState: ShieldState.ACTIVE,
  isHardMode: false, // Add hard mode to game state
};
...
app.use((req, res, next) => {
  if (req.url === '/') {
    // Redirect root to index.html with version parameter
    res.redirect('/index.html?v=7');
  } else {
    next();
  }
});
...
function broadcastState() {
  ...
  const anyPrimaryPressed = primaryModule.primaryButtonStates.some(
    (state) => state.pressed,
  );
  const firstPressedTime =
    primaryModule.primaryButtonStates.find((state) => state.pressed)
      ?.pressTime || null;
  ...
  const state = {
    ...
    isRegenerating:
      primaryModule.state.shieldState === ShieldState.REGENERATING,
    currentState: primaryModule.state.shieldState,
    isHardMode: primaryModule.state.isHardMode,
  };
    ...
    wss.on('connection', (ws) => {
      ...
      ws.on('message', (message) => {
        try {
          ...
          if (data.type === 'toggleDifficulty') {
            if (primaryModule && primaryModule.state) {
              primaryModule.state.isHardMode = !primaryModule.state.isHardMode;
              gameState.isHardMode = primaryModule.state.isHardMode;
              broadcastState();
              console.log(
                'Difficulty mode toggled:',
                primaryModule.state.isHardMode ? 'Hard' : 'Normal',
              );
            }
          }
        } catch (error) {
